...
For Almere's structured concept input to the regional workagenda process, use `docs/workagenda-almere-regional-input-model.md`, `data/workagenda/almere_regional_workagenda_input_objects.json`, `data/schemas/almere_regional_workagenda_input.schema.json`, and `src/build_almere_regional_workagenda_input.py`. This generated layer translates the D5 status matrix and validation tickets into Almere handoff objects with `current_state`, `target_state_for_regional_workagenda`, `gap_summary`, `decision_requests_for_region`, `validation_needed_before_or_after_submission`, and `handoff_to_region`. It is not a source document, not the regional workagenda, not a confirmed Almere position and not validation proof. The 15 September 2026 Almere submission target is a planning assumption; the 15 November 2026 deadline is the source-backed regional adoption deadline.
