...
- Check each claim's `money_status` before using finance language; `funding_route`, `application_condition`, `budget_window`, `allocation_mechanism`, `spending_scope`, `accountability_rule`, `local_funding_gap`, `double_counting_risk`, `finance_context`, and `not_financial` require different public wording.
...
- Check each claim's `locality_status` before using Almere/Flevoland language; `explicit_almere`, `explicit_flevoland`, `regional_split_context`, `national_general`, `national_with_local_relevance`, `inferred_local_relevance`, `local_adoption_gap`, `municipal_context`, and `no_locality_signal` require different public wording.
