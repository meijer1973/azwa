...
| 32.1 D5-statusmatrix bouwen | open | Eerste matrixlaag staat in `docs/workagenda-d5-statusmatrix.md` en `data/workagenda/d5_status_matrix.json`; gebruikt `src/build_workagenda_d5_status_matrix.py` en `tests/test_workagenda_d5_status_matrix.py`. |

## Huidige sprintplan
Sprint 32.1 is open. De lopende taak is de D5-statusmatrix gebruiken als centrale stuurlaag voor werkagenda-readiness: per D5-onderdeel zichtbaar maken welke velden brononderbouwd zijn, welke validatie nodig hebben, welke finance/controller of Zvw-bevestiging vragen, welke besluitvorming ontbreken en welke D6-afhankelijkheden zichtbaar moeten blijven. Sprint 32.0 is afgerond. Sprint 25.6 blijft geparkeerd omdat post-validatie hardening pas verantwoord is na stakeholdervalidatierecords, lokale/interne documenten, finance/controller bevestiging of een expliciet beleidsbesluit.
